...
func listReceipts(ctx context.Context, client *appie.Client, n int) error {
	if n < 0 {
		return fmt.Errorf("-n must be >= 0, got %d: %w", n, errBadArgs)
	}
	...
	// "No receipts found" applies only when the upstream returned zero — not
	// when the user explicitly capped the view with -n 0.
	if len(receipts) == 0 {
